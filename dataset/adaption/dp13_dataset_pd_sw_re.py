...
class densepass13ValDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(2048, 400), sw_setting=None, mean=(128, 128, 128),
                 scale=False, mirror=False, ignore_label=255, set='val'):
        self.root = root
        self.list_path = list_path
...
class densepass13TestDataSet_new(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(2048, 400), sw_setting=None, mean=(128, 128, 128),
                 scale=False, mirror=False, ignore_label=255, set='val'):
        self.root = root
        self.list_path = list_path
    ...
    def __getitem__(self, index):
        ...
        # **4. 进行 Sliding Window**
        image_patches, label_patches = self._sliding_windows(
            image, label, 
            window_size=self.sw_size, 
            h_stride=self.sw_h_stride, 
            w_stride=self.sw_w_stride
        )
        print(image_patches.shape)
        print(self._coords_list())
        # **5. Resize 到 1024x1024**
        resize_transform = transforms.Resize((1024, 1024), interpolation=Image.BICUBIC)
        resize_label_transform = transforms.Resize((1024, 1024), interpolation=Image.NEAREST)

        resized_images = [resize_transform(transforms.ToPILImage()(img)) for img in image_patches]
        resized_labels = [resize_label_transform(transforms.ToPILImage()(lbl.to(torch.uint8))) for lbl in label_patches]

        # **6. 重新转换为 Tensor**
        image_patches = torch.stack([input_transform(img) for img in resized_images])
        label_patches = torch.stack([torch.LongTensor(np.array(lbl).astype('int32')) for lbl in resized_labels])

        size = np.asarray(label_patches).shape  # 计算尺寸信息

        return image_patches, label_patches, size, name, ori_label
